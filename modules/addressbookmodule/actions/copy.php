...
// PERM CHECK
	...
	if (count($othersources)) {
		...
		$form->register("submit","",new buttongroupcontrol("Copy","","Cancel"));
		$form->location($loc);
		$form->meta("contact_id",$_GET['id']);
		$form->meta("action","save_copy");
		
		
		$template = new template("addressbookmodule","_form_copy",$loc);
		$template->assign("form_html",$form->toHTML());
		$template->assign("ref_link","?module=addressbookmodule&src=".$loc->src."&int=".$loc->int."&action=reference&id=".$_GET['id']); // REMOVELINK
		$template->output();
	} else {
