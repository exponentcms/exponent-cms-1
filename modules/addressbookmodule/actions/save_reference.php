...
if ($contact) {
	...
	if ((pathos_permissions_check("reference",$loc) || pathos_permissions_check("reference",$iloc)) &&
		pathos_permissions_check("post",$dloc)
	) {
		$newc = null;
		$newc->copy_id = $contact->id;
		$newc->location_data = serialize($dloc);
		$db->insertobject($newc,"addressbook_contact");
