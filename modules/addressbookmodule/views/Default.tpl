...
<div class="addressbookmodule default">
	<h2>{$moduletitle}</h2>
				...
				<td>
					{permissions level=$smarty.const.UILEVEL_PERMISSIONS}
						{if $permissions.administrate == true || $contact->permissions.administrate == true}
							<a href="{link action=userperms int=$contact->id _common=1}"><img class="mngmnt_icon" style="border:none;" src="{$smarty.const.ICON_RELATIVE}userperms.png" title="{$_TR.alt_userperm_one}" alt="{$_TR.alt_userperm_one}" /></a>&nbsp;
							<a href="{link action=groupperms int=$contact->id _common=1}"><img class="mngmnt_icon" style="border:none;" src="{$smarty.const.ICON_RELATIVE}groupperms.png" title="{$_TR.alt_groupperm_one}" alt="{$_TR.alt_groupperm_one}" /></a>
						{/if}
					{/permissions}
					...
					<a class="mngmntlink addressbook_mngmntlink" href="{link action=view id=$contact->id}">
						<img class="mngmnt_icon" style="border:none;" src="{$smarty.const.ICON_RELATIVE}view.png" title="{$_TR.alt_view}" alt="{$_TR.alt_view}" />
					</a>
				</td>
			</tr>
		{foreachelse}
