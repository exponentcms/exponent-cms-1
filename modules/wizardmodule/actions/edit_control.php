...
$f = $db->selectObject("wizard_form","id=".(isset($_REQUEST['form_id'])?intval($_REQUEST['form_id']):0));
if ($f) {
	//if (exponent_permissions_check("editform",unserialize($f->location_data))) {
		if (isset($_POST['control_type']) && $_POST['control_type']{0} == ".") {
			...
			$ctl->form_id = $f->id;
			$ctl->is_readonly = 1;
			if (!$db->countObjects("wizard_control","form_id=".$f->id)) $ctl->rank = 0;
			else $ctl->rank = $db->max("wizard_control","rank","form_id","form_id=".$f->id)+1;
			$db->insertObject($ctl,"wizard_control");
			exponent_flow_redirect();
		} else {
			$control_type = "";
			$ctl = null;
			if (isset($_GET['id'])) {
				$control = $db->selectObject("wizard_control","id=".intval($_GET['id']));
				if ($control) {
