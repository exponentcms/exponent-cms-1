...
$template = new template('wizardmodule','_data_view');
exponent_flow_set(SYS_FLOW_PUBLIC,SYS_FLOW_ACTION);

if (isset($_GET['id'])) {
	$_GET['id'] = intval($_GET['id']);
	
	$f = $db->selectObject("wizard_form","id=".$_GET['id']);
	$rpt = $db->selectObject("wizard_report","form_id=".$_GET['id']);
	$items = $db->selectObjects("wizard_".$f->table_name);
	if (exponent_permissions_check("viewdata",unserialize($f->location_data))) {
		...
		if ($rpt->column_names == '') {
			...
			$controls = $db->selectObjects("wizard_control","form_id=".$f->id." and is_readonly = 0 and is_static = 0");
		...
		foreach (explode("|!|",$rpt->column_names) as $column_name) {
			...
			elseif ($column_name == "timestamp") {
				...
				$control = $db->selectObject("wizard_control","name='".$column_name."' and form_id=".$_GET['id']);
				if ($control) {
