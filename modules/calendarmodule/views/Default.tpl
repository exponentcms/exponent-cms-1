		...
		<td>
			{permissions level=$smarty.const.UILEVEL_PERMISSIONS}
				{if $permissions.administrate == 1 || $item->permissions.administrate == 1}
					<a class="mngmntlink calendar_mngmntlink" href="{link action=userperms int=$item->id _common=1}"><img border="0" src="{$smarty.const.ICON_RELATIVE}userperms.gif" title="Assign user permissions on this Calendar Event" alt="Assign user permissions on this Calendar Event" /></a>
					<a class="mngmntlink calendar_mngmntlink" href="{link action=groupperms int=$item->id _common=1}"><img border="0" src="{$smarty.const.ICON_RELATIVE}groupperms.gif" title="Assign group permissions on this Calendar Event" alt="Assign group permissions on this Calendar Event" /></a>
				{/if}
			{/permissions}
			{permissions level=$smarty.const.UILEVEL_NORMAL}
				{if $permissions.edit == 1 || $item->permissions.edit == 1}
					{if $item->approved == 1}
					<a class="mngmntlink calendar_mngmntlink" href="{link action=edit id=$item->id date_id=$item->eventdate->id}"><img border="0" src="{$smarty.const.ICON_RELATIVE}edit.gif" title="Edit this Calendar Event" alt="Edit this Calendar Event" /></a>
					{else}
				...
				{if $permissions.delete == 1 || $item->permissions.delete == 1}
					...
					<a class="mngmntlink calendar_mngmntlink" href="{link action=delete id=$item->id}" onClick="return confirm('Are you sure you want to delete this Calendar Event?');"><img border="0" src="{$smarty.const.ICON_RELATIVE}delete.gif" title="Delete this Calendar Event" alt="Delete this Calendar Event" /></a>
					{else}
					<a class="mngmntlink calendar_mngmntlink" href="{link action=delete_form id=$item->id date_id=$item->eventdate->id}"><img border="0" src="{$smarty.const.ICON_RELATIVE}delete.gif" title="Delete this Calendar Event" alt="Delete this Calendar Event" /></a>
					{/if}
					{else}
...
{if $modconfig->enable_categories == 1}
<a href="{link action=cat_managecategories}" class="mngmntlink calendar_mngmntlink">Manage Categories</a>
{/if}