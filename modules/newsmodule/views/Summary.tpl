...
{if $permissions.administrate == 1}
	<a href="{link action=userperms _common=1}"><img border="0" src="{$smarty.const.ICON_RELATIVE}userperms.gif" title="Assign user permissions on this News Feed System" alt="Assign user permissions on this News Feed System" /></a>&nbsp;
	<a href="{link action=groupperms _common=1}"><img border="0" src="{$smarty.const.ICON_RELATIVE}groupperms.gif" title="Assign group permissions on this News Feed System" alt="Assign group permissions on this News Feed System" /></a>
{/if}
{if $permissions.configure == 1}
	<a href="{link action=configure _common=1}"><img border="0" src="{$smarty.const.ICON_RELATIVE}configure.gif" title="Change the configuration of this News Feed System" alt="Change the configuration of this News Feed System" /></a>
{/if}
...
{foreach from=$news item=newsitem}
	<div>
		...
		{if $permissions.edit_item == true || $newsitem->permissions.edit_item == true}
			...
			{else}
			<a class="mngmntlink news_mngmntlink" href="{link action=edit id=$newsitem->id}"><img border="0" src="{$smarty.const.ICON_RELATIVE}edit.gif" title="Edit this News Item" alt="Edit this News Item" /></a>
			{/if}
		{/if}
		{if $permissions.delete_item == true || $newsitem->permissions.delete_item == true}
			{if $newsitem->approved == 2} {* in ap *}
			<img border="0" src="{$smarty.const.ICON_RELATIVE}delete.disabled.gif" title="Deleting Disabled - News Item In Approval" alt="Deleting Disabled - News Item In Approval" />
			{else}
			<a onClick="return confirm('Are you sure you want to delete this news item?');" class="mngmntlink news_mngmntlink" href="{link action=delete id=$newsitem->id}"><img border="0" src="{$smarty.const.ICON_RELATIVE}delete.gif" title="Delete this News Item" alt="Delete this News Item" /></a>
			{/if}
		{/if}
