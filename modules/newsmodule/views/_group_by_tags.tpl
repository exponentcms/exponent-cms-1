...
{foreach from=$tag item=newsitem}
	...
	<div>
		<li class="news_itemtitle">
			...
			{$_TR.posted}{$newsitem->posted|format_date:"%B %d, %Y"}<br />[<a href="{link action=view id=$newsitem->id}">{$_TR.read_more}<span> "{$newsitem->title}"</span></a>]
		</li>
		{permissions level=$smarty.const.UILEVEL_PERMISSIONS}
		...
		{if $permissions.manage_approval == 1}
			<a class="mngmntlink news_mngmntlink" href="{link module=workflow datatype=newsitem m=newsmodule s=$__loc->src action=revisions_view id=$newsitem->id}" title="{$_TR.alt_revisions}">{$_TR.revisions}</a>
		{/if}
		{/permissions}
	</div>
	</ul>
