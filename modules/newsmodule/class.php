...
class newsmodule {
	...
	function show($view,$loc = null,$title = "") {
		...
		$news = $db->selectObjects("newsitem","location_data='" . serialize($loc) . "' AND (publish = 0 or publish <= " . time() . ") AND (unpublish = 0 or unpublish > " . time() . ") AND approved != 0 ORDER BY ".$config->sortfield." " . $config->sortorder . " LIMIT 0,".$config->item_limit);
