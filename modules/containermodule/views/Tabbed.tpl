...
<div id="nav-tabs" class="yui-navset">
	...
	<div class="yui-content">
		{section name=contain loop=$numcontainers+1}	
			...
			{if $container != null}	
				<div id="tab{math equation="x + y" x=$smarty.section.contain.index y=1}"{if !$smarty.section.contain.first} style="display:none"{/if}>
						...
						{permissions level=$smarty.const.UILEVEL_STRUCTURE}
							{if ($permissions.administrate == 1 || $permissions.edit_module == 1 || $permissions.delete_module == 1 || $permissions.add_module == 1 || $container->permissions.administrate == 1)}
								<div id="module{$container->id}" class="container_modulewrapper">
									...
									YAHOO.expadminmenus["{$container->info.class}{$container->id}"] = {getchromemenu module=$container rank=$menurank rerank='false'}; 
									{/script}
							{/if}
						{/permissions}
						{$container->output}
