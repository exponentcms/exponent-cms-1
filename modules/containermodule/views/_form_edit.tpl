	...
	<fieldset id="contentsourcecontrol" class="control">
		<legend>{$_TR.source}</legend>
			<label id="sourceControl">
				<input type='radio' class="radiobuttoncontrol" name='i_src' value='new_source' id='r_new_source' onclick='activate("New");' />
				<span class="label">{$_TR.new_content}</span>
			</label>
			<label id="sourceControl">
...
{/if}
	<fieldset id="titleControl" class="control">
		...
		<input type="text" class="textcontrol" name="title" id="title" value="{$container->title}" onchange="showPreviewCall()" />
	</fieldset>
<fieldset id="descriptionControl" class="control">
	<legend>{$_TR.description}</legend>
	<textarea rows="5" cols="30" class="textareacontrol" id="ta_description" {if $container->is_existing}disabled{/if} name="description">{$locref->description}</textarea>
</fieldset>
<label id="buttonControl" class="control">
