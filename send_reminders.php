...
//define('SCRIPT_FILENAME','send_reminders.php');
...
//		$config->enable_categories = 0;
	...
	$view = $_GET['view'];
	if ($view == "") {
		$view = "_reminder_week";	// default reminder view
	}
	
// /// seed these until they get put in the database/form
	// $config->email_title_reminder = "Calendar Reminder";
	// $config->email_from_reminder = "Calendar Manager";
	// $config->email_address_reminder = "events@".HOSTNAME;
	// $config->email_reply_reminder = "events@".HOSTNAME;
	// $config->email_showdetail = 0;
...
// format and send email
	...
	exponent_smtp_mail($emails,$from_addr,$subject,$msg,$headers);
	*/
	$emails = array();
	foreach ($notifs as $n) {
	//			if ($n->user_id != $user->id) {
	...
	// finally, display a web page with the result of the action?
