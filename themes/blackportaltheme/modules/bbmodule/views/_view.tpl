...
<tr class="bb_row">
	<!-- td class="bb_threadlist"><img style="border:none" src="{$smarty.const.ICON_RELATIVE}thread_icon.gif" title="{$_TR.alt_thread}" alt="{$_TR.alt_thread}"/></td -->
	<td class="bb_threadlist" valign="middle">
		<div style=" float:left; width:25px; height:20px; ">
		<img style="text-align:left" src="{$smarty.const.ICON_RELATIVE}announce16x16.gif" title="{$_TR.alt_announce}" alt="{$_TR.alt_announce}" />
		</div>
	...
	<td class="center">{attribution user=$announcement->user}</td>
	<td class="center"><span class="bb_date">{$announcement->posted|format_date:"%D %T"}</span></td>
	<td class="center"><span class="bb_date">{$announcement->updated|format_date:"%D %T"}</span></td>
</tr>
{/foreach}
...
<tr class="bb_row">
	...
	<td class="center">{attribution user=$sticky->user}</td>
	<td class="center"><span class="bb_date">{$sticky->posted|format_date:"%D %T"}</span></td>
	<td class="center"><span class="bb_date">{$sticky->updated|format_date:"%D %T"}</span></td>
</tr>
{/foreach}
...
<tr class="bb_row">
	...
	<td class="center">{attribution user=$thread->user}</td>
	<td class="center"><span class="bb_date">{$thread->posted|format_date:"%D %T"}</span></td>
	<td class="center"><span class="bb_date">{$thread->updated|format_date:"%D %T"}</span></td>
</tr>
{foreachelse}
<tr>
