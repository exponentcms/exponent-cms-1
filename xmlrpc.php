...
function getUsersBlogs($xmlrpcmsg)
{
	...
	if (userLogin($username, $password, null, 'post') == true) {	
...
function newPost($xmlrpcmsg)
{
    ...
    if(userLogin($username, $password, $src, 'post') == true) {
		$loc = exponent_core_makeLocation('weblogmodule',$src);	
		if (exponent_permissions_check('post',$loc)) {
...
function editPost($xmlrpcmsg)
{
	...
	$post = $db->selectObject('weblog_post','id='.intval($postid));
...
//			if (!exponent_permissions_check('view_private',$loc)) $where .= ' AND is_private = 0';
			...
			$posts = $db->selectObjects('weblog_post',$where . ' ORDER BY posted DESC '.$db->limit($numposts,0));
//echo print_r($posts);
...
//				  'categories'        => new xmlrpcval(array(new xmlrpcval($posts[$i]->selected_tags, 'string')), 'array'),
				  'publish'        => new xmlrpcval((($posts[$i]->is_draft) ? 0 : 1), 'boolean')
				  ), 'struct'); 	
			}
