...
/*
	...
	$dh = opendir($linksrc);
	while (($file = readdir($dh)) !== false) {
		if (is_dir("$linksrc/$file") && substr($file,0,1) != "." && $file != "CVS") {
			
			mkdir("$linkdest/$file");
			pathos_sharedcore_copyDirectoryStructure("$linksrc/$file","$linkdest/$file");
		}
	}
*/
}
...
function pathos_sharedcore_linkFile($linksrc,$linkdest) {
	...
	if (!is_file($linksrc)) {
		return SHAREDCORE_ERR_BADFILETYPE;
	}
	
	$destdir = dirname($linkdest);
	$srcfile = basename($linksrc);

	if (file_exists("$destdir/$srcfile")) {
		return SHAREDCORE_ERR_LINKDEST_EXISTS;
	}
...
function pathos_sharedcore_linkFiles($src,$dest) {
	if (is_readable($src) && is_writable($dest)) {
		$dh = opendir($src);
		while (($file = readdir($dh)) !== false) {
			if (is_file("$src/$file") && substr($file,0,1) != ".") {
	...
	if (is_readable($src) && is_writable($dest)) {
		...
		while (($file = readdir($dh)) !== false) {
			if (substr($file,0,1) != "." && $file != "CVS") {
				// Hidden files and . .. dirs should never be linked
				// CVS dev files should never be linked.
				if (is_dir("$src/$file") && $recurse) {
					mkdir("$dest/$file");
					pathos_sharedcore_linkDirectory("$src/$file","$dest/$file",true);
				}
				else {
					symlink("$src/$file","$dest/$file");
				}
...
function pathos_sharedcore_listDirectory($dir,$recurse = false) {
	...
	while (($file = readdir($dh)) !== false) {
		if (substr($file,0,1) != "." && $file != "CVS") {
			if (is_dir("$dir/$file")) {
				$arr[$file] = pathos_sharedcore_listDirectory("$dir/$file");
			} else {
				$arr[$file] = $file;
			}
		}
